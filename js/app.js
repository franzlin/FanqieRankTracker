document.addEventListener('DOMContentLoaded', () => {
    ...
    channelBtns.forEach(btn => {
        btn.addEventListener('click', () => {
            ...
            // Re-render with channel filter
            if (allData) {
                currentCategory = null; // reset selection
                applyData(allData);
            }
        });
    });

    // ========== Helpers: channel & cat_key ==========
    function catChannel(cat) {
        return cat.channel || 'female';
    }

    function catKey(cat) {
        return `${catChannel(cat)}:${cat.name}`;
    }

    function filterCategories(cats) {
        if (currentChannel === 'all') return cats;
        return cats.filter(c => catChannel(c) === currentChannel);
    }

    function findTrend(trendData, cat) {
        if (!trendData) return {};
        const ck = catKey(cat);
        // Try channel:name format (new), then plain name (old)
        return trendData[ck] || trendData[cat.name] || {};
    }
    ...
    // Try new filename first, fall back to old format
    function snapshotUrl(dateStr) {
        const fileDateStr = dateStr.replace(/-/g, '');
        return `data/fanqie_new_ranks_${fileDateStr}.json`;
    }

    function snapshotUrlLegacy(dateStr) {
        const fileDateStr = dateStr.replace(/-/g, '');
        return `data/fanqie_female_new_ranks_${fileDateStr}.json`;
    }
    ...
    function loadDateData(dateStr) {
        ...
        const newUrl = snapshotUrl(dateStr);
        const oldUrl = snapshotUrlLegacy(dateStr);
        const trendUrl = `data/trends/${dateStr}.json?${cacheBuster}`;

        // Try new filename first, fall back to legacy
        function fetchSnapshot(url) {
            return fetch(`${url}?${cacheBuster}`).then(r => {
                if (!r.ok) throw new Error('Not found');
                return r.json();
            });
        }

        fetchSnapshot(newUrl)
            .catch(() => fetchSnapshot(oldUrl))
            .then(snapshot => {
                return Promise.all([
            ...
            .then(([snapshot, trendData]) => {
                ...
                const combined = {
                    ...
                    categories: snapshot.categories.map(cat => ({
                        ...
                        trend: findTrend(trendData && trendData.trends, cat),
                        books: cat.books || []
                    }))
                };
                allData = combined;
    ...
    function applyData(data) {
        ...
        const filtered = filterCategories(data.categories);

        // Try to restore previously selected category, otherwise pick first
        const categoryExists = savedCategory && filtered.some(c => catKey(c) === savedCategory);
        if (categoryExists) {
            selectCategory(savedCategory);
            // Also update sidebar active state
            document.querySelectorAll('#category-list li').forEach(el => {
                el.classList.toggle('active', el.dataset.catKey === savedCategory);
            });
        } else if (filtered.length > 0) {
            selectCategory(catKey(filtered[0]));
        } else {
            // No categories in this channel
            waterfall.innerHTML = '<p style="color:var(--text-muted);padding:20px;">该频道暂无数据。</p>';
    ...
    function renderCategories() {
        ...
        const filtered = filterCategories(allData.categories);

        // Group by channel for display when "all" is selected
        let groups;
        if (currentChannel === 'all') {
            groups = [
                { label: '女频', cats: filtered.filter(c => catChannel(c) === 'female') },
                { label: '男频', cats: filtered.filter(c => catChannel(c) === 'male') },
            ].filter(g => g.cats.length > 0);
        } else {
            groups = [{ label: '', cats: filtered }];
        }
        ...
        groups.forEach(group => {
            ...
            group.cats.forEach((cat, i) => {
                const ck = catKey(cat);
                const li = document.createElement('li');
                li.dataset.catKey = ck;
                ...
                if ((currentCategory && ck === currentCategory) || (!currentCategory && i === 0 && group === groups[0])) {
    ...
    // ========== Select a category ==========
    function selectCategory(ck) {
        currentCategory = ck; // persist selection as cat_key
        const cat = allData.categories.find(c => catKey(c) === ck);
        if (!cat) return;
    ...
    function renderBooks(cat) {
        ...
        const changeMap = buildPrevRankMap(catKey(cat));
