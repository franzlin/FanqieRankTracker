document.addEventListener('DOMContentLoaded', () => {
    ...
    async function init() {
        ...
        try {
            ...
            const snapshots = await Promise.all(
                dates.map(date => fetchSnapshot(date))
            );
    ...
    function snapshotUrl(date) {
        return `data/fanqie_new_ranks_${date.replace(/-/g, '')}.json`;
    }

    function snapshotUrlLegacy(date) {
    ...
    // Try new filename first, fall back to legacy female-only format
    function fetchSnapshot(date) {
        return fetchJson(`${snapshotUrl(date)}?${cacheBuster}`)
            .catch(() => fetchJson(`${snapshotUrlLegacy(date)}?${cacheBuster}`))
            .catch(() => null);
    }
